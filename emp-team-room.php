...
// Take the lowest id value for team name as id
$teamIdSql = "SELECT MIN(id) AS min_id FROM teams WHERE team_name='$currentTeam'";
$teamIdResult = mysqli_query($link, $teamIdSql);
$row = mysqli_fetch_assoc($teamIdResult);
$minId = $row['min_id'];

// Fetch calendar data and convert to JSON before displaying on FullCalendar
$teamCalSql = "SELECT event_name, event_start, event_end FROM team_calendar WHERE teamCID='$minId'";
