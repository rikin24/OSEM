...
if (isset($_GET['id'])) {
    ...
    if (empty($eventName)) {
        header("Location: ../man-ind-calendar-update.php?id=$id&error=Missing Event Name");
    } else if (empty($eventStart)) {
        header("Location: ../man-ind-calendar-update.php?id=$id&error=Missing Event Start Date");
    } else if (empty($eventEnd)) {
        header("Location: ../man-ind-calendar-update.php?id=$id&error=Missing Event End Date");
    } else if ($eventStart > $eventEnd) {
        header("Location: ../man-ind-calendar-update.php?id=$id&error=Event Start Date Cannot Be Later Than End Date");
    } else {
        ...
        if ($result) {
            header("Location: ../man-ind-calendar-update.php?id=$id&success=Event Updated Successfully");
        } else {
